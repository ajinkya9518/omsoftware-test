...
class SignUpForm(UserCreationForm):
    ...
    '''
    mob_no = forms.CharField((label=_('mob_no'), help_text=_('Required. Enter an existing mobile no.'))

    def clean_mob_no(self):
        mob_no = self.cleaned_data['mob_no']

        user = User.objects.filter(mob_no__iexact=mob_no).exists()
        if user:
            raise ValidationError(_('You can not use this mobile no'))
